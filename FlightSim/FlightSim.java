...
class FlightWindow extends JFrame{
    ...
    public String BaseStarDataPath = RunDirectory + "FlightStars//";
    ...
    private void loadStars(){
        StarData = FileHandler.readFile(BaseStarDataPath + "Stars.txt", "\t");
    }
    ...
    public void addFlightState(String Path, int Index){
    	FlightState FS = new FlightState(this, Path, StarData, Index, LevelMenuS);
        addState(FS);
...
class FlightState extends GameState implements KeyListener, ActionListener, MouseListener, MouseMotionListener, MouseWheelListener{
    ...
    class ObjectVector{
        private Vector V;
        public final boolean contains(Object3DM A){
            return V.contains(A);
        }
        public ObjectVector(){
            V = new Vector();
        }
        public final Object3DM obj(int INTA){
            return (Object3DM)V.get(INTA);
        }
        public final Doodad d(int INTA){
            return (Doodad)V.get(INTA);
        }

        public final Plane pl(int INTA){
            return (Plane)V.get(INTA);
        }

        public final Craft cr(int INTA){
            return (Craft)V.get(INTA);
        }

        public final Turret tu(int INTA){
            return (Turret)V.get(INTA);
        }

        public final Vehicle ve(int INTA){
            return (Vehicle)V.get(INTA);
        }

        public final TempSprite ts(int INTA){
            return (TempSprite)V.get(INTA);
        }

        public final Projectile pr(int INTA){
            return (Projectile)V.get(INTA);
        }

        public final Trail tr(int INTA){
            return (Trail)V.get(INTA);
        }

        public final Position3DM ps(int INTA){
            return (Position3DM)V.get(INTA);
        }

        public final Object3DM lastElement(){
            return (Object3DM)V.get(V.size() - 1);
        }
        public final boolean add(Object3DM A){
            V.add(A); 
            A.Exists = true;        
            return true;
        }
        public final int addAll(Object3DM[] A){
            int i = 0;
            for(Object3DM a : A){
                V.add(a);
                i++;
        ...
        public final void removeElementAt(int A){
        ...
        public final boolean remove(Object3DM A){
        ...
        public final int indexOf(Object3DM O){
            return V.indexOf(O);
        }
    }
    ...
    class Plane extends Object3DM{
       // Color Tint = Color.green.darker();;
        Point3DM Normal;
      ...
      //  Point3DM[] EdgeUnits; //Unit Vector from Point N+1 to Point N
        ...
        public Plane(Point3DM[] C){
            ...
            int[] PlaneArray = new int[C.length];
            for(int i = 0; i < PlaneArray.length; i++){
            	PlaneArray[i] = i;	
            }
            if(Finite){
            	Form.Planes.add(PlaneArray);
            }
            Form.finalizeLinks();
        ...
        public Point3DM getNormalFacing(Point3DM Target){
            if(Normal.dot(Target.subtract(Form.Center)) > 0){
        ...
        public void doCollision(Doodad Target, Point3DM ImpactSpot){
            ...
            Point3DM EffectiveNormal = getNormalFacing(Target.Form.Center);
             
          
            if(EffectiveNormal.dotProduct(Target.Momentum) < 0){
                Normal.multiply(Normal.dotProduct(Target.Momentum), Difference);
                Difference.multiply(-1.0 - Target.BounceFactor, Difference);
                Target.Momentum.add(Difference, Target.Momentum);
                
            }
   
            if(Target.Form.Center.subtract(ImpactSpot).length() < Target.CollisionRadius){
                ImpactSpot.add(Target.Form.Center.subtract(ImpactSpot).normalize(Target.CollisionRadius), Target.Form.Center);
            }
         
            //System.out.println(Target.Momentum);
        }
        public void doCollision(Vehicle Target, Point3DM ImpactSpot){
            ...
            if(ImpactSpot.subtract(Target.Form.Center).dotProduct(Target.Momentum) > 0){
             ...
             //   System.out.println(Temp.length());
                ...
                if(RemovedSpeed > 10)fragment(ImpactSpot, Target, RemovedSpeed);
        ...
        public void fragment(Point3DM ImpactSpot, Object3DM Target, double RemovedSpeed){
            Doodad[] Debris = createDebris(15, ImpactSpot, Utils3DM.Origin, Normal.dot(Target.Form.Center.subtract(ImpactSpot)) > 0 ? Normal : Normal.multiply(-1), new Color[]{this.Form.Tint, Target.Form.Tint}, 1, Target.Form.MaxDistance * RemovedSpeed / 100, 1);
            Doodads.addAll(Debris);
        }
        ...
        final Point3DM CDifference = new Point3DM();
        final Point3DM Temp = new Point3DM();
        public Point3DM checkCollision(Object3DM Target){
            ...
            Center.subtract(Form.Center, CDifference);

            if(CDifference.length() > CollisionRadius + Form.MaxDistance){
                return null;
            }

            //Gets perpendicular projection of center of craft onto infinite plane
            Normal.multiply(CDifference.dotProduct(Normal), SurfaceImage);
            Center.subtract(SurfaceImage, SurfaceImage);

            //Calculates distance of craft to perpendicular plane
            Center.subtract(SurfaceImage, CDifference);

            if(CDifference.length() > CollisionRadius){
            ...
            for(int a = 0; a < Form.ActualPoints.length; a++){
                Center.subtract(Form.ActualPoints[a], CDifference);
                if(CDifference.length() <= CollisionRadius){
    ...
    class Gun{
        ...
        public Gun copy(Object3DM base, Point3DM[] pos, Rotation3DM facing){
            Gun G = new Gun(Name, base, pos, facing, Shot, Ammo, ReloadTimer.Max, ReloadTimer.BaseIncrement, CooldownCounter.Max);
            return G;
            
        }
        public Gun(String N, Object3DM base, Point3DM[] pos, Rotation3DM facing, ProjectileType shot, int ammo, double maxr, double rr, double Cooldown){
           Name = N;
           Base = base;
           POS = pos;
           Facing = facing;
           ...
           CooldownCounter = new Counter(0, Cooldown, Cooldown);
        }
        public void reload(){
    ...
    class ProjectileType extends Object3DM{
        ...
        public ProjectileType(double M, double CR, double D, boolean R, double S, double DMG){
            Duration = (float)D;
            Speed = (float)M;
            CollisionRadius = CR;
            Rotating = R;
        ...
        public void setTrail(double TT, int Hook, int J){
            TrailTime = (float)TT;
            Joints = J;
            Attachment = Hook;
        }
        
        public void setAIData(double MER, double MER2, double MTS, double MTS2, int AIPreference){
            MaxEffectiveRange = (float)MER;
            MinEffectiveRange = (float)MER2;
            MaxTargetSpeed = (float)MTS;
            MinTargetSpeed = (float)MTS2;
        ...
        public Projectile createProjectile(Object3DM Base, Point3DM Position, Rotation3DM Facing, Object3DM Target){
          //  System.out.println("RAR");
            ...
            if(Store.size() == 0){        
                ...
                P.init(Base, Form.copy(), Position, Direction.multiply(Speed).add(Base.Momentum), Duration, Form.Tint);    
            }else{
            ...
            Direction.multiply(Spread * Speed, Direction);
            ...
            P.Form.calcPoints();
            P.Target = Target;
     //       System.out.println(Arrays.toString(P.BulletTrail.Form.ActualPoints));
            return P;
    ...
    class Projectile extends Object3DM{
        ...
        final Trail BulletTrail;
        final Point3DM LastPosition;
   
        
        Counter Duration;

        final Counter HomingDelay;
        final Rotation3DM Turn;
        ...
        public Projectile(ProjectileType PT){
            Type = PT;
            Form = Type.Form.copy();
            HomingDelay = new Counter();
            
            CollisionRadius = Type.CollisionRadius;
            Turn = new Rotation3DM();
            Form.Center = new Point3DM();
            LastPosition = new Point3DM();
            BulletTrail = new Trail(this, Type.Joints, Form.Center, Form.Tint, Type.TrailTime);
            Duration = new Counter();
        }
        public void init(Object B, Shape3DM form, Point3DM Center, Point3DM V, double D, Color T){
            Base = B;

            Center.copyTo(Form.Center);

            Center.copyTo(LastPosition);

            Form.Tint = T;

            Momentum = V.copy();

            Duration.reset(0, D, D);

            HomingDelay.reset(0, Type.HomingDelay, Type.HomingDelay);
        ...
        public void turnProjectile(){
          //  System.out.println(Momentum);
           ...
           // if(Base == Player)System.out.println(projectileJudgeLead().length());
            Rotation3DM.getRotationBetween(Momentum, Direction, Type.TurnPerSecond * 1.0 / FramesPerSecond, Turn);
            Rotation3DM.multiply(Turn, Form.Rotation, Form.Rotation);
            
            Form.Rotation.getOriginalAxis('x', Momentum);
            Momentum.multiply(Type.Speed, Momentum);
        ...
        public Point3DM judgeDirection(){
            ...
            double T = GUI.newtonRaphson(0.25 * A.dot(A), 0.5 * V.dot(A), V.dot(V) - P * P + 0.5 * D.dot(A), D.dot(V), D.dot(D), Form.Center.distanceTo(Target.Form.Center) / Type.Speed, 15, 0.01);
            if(T == T){
                Point3DM PCap = D.add(V.multiply(T)).add(A.multiply(0.5 * T*T)).divide(Type.Speed * T);
        ...
        public void move(){
           ...
           // if(HomingDelay.checkBounds() >= 0 && Homing){
            ...
            //}
            if(Type.Homes && Target != null && HomingDelay.checkBounds() < 0){
                turnProjectile();
            }
            Acceleration = new Point3DM(0, 0, -Type.Gravity);
            Momentum.add(Acceleration.divide(FramesPerSecond), Momentum);
      ...
      //   System.out.println(Form.Center + "\t" + Duration);
        ...
        public void fragment(Point3DM ImpactSpot, Object3DM Target){
            Point3DM Normal = !(Target instanceof Plane) ? null : ((Plane)Target).getNormalFacing(LastPosition);
            System.out.println(Normal);
            Doodad[] Debris = createDebris(3, ImpactSpot, Utils3DM.Origin, Normal, new Color[]{this.Form.Tint, Target.Form.Tint}, 1, 2*Type.Damage, 3);
            Doodads.addAll(Debris);
        }
        public void doCollision(Plane Target, Point3DM ImpactSpot){
            playSound("VehicleHitExplosion", Form.Center, Type.Damage);
            die(ImpactSpot);
            fragment(ImpactSpot, Target);
        ...
        public Point3DM checkCollision(Object3DM Target){
            ...
            if(Form.Center.distanceTo(Target.Form.Center) > CollisionRadius + Target.CollisionRadius + Type.Speed * 1.0 / FramesPerSecond){
                return null;
            }
        ...
        public void die(Point3DM ImpactSpot){
            ...
            if(Type.AreaOfEffect != 0) for(int i = 0; i < Vehicles.size(); i++){
                Vehicle V = Vehicles.ve(i);
                if(V.Form.Center.distanceTo(ImpactSpot) < Type.AreaOfEffect){
                    V.hit(V.Form.Center.add(ImpactSpot.subtract(V.Form.Center).normalize(V.CollisionRadius)), Type.Damage);
                    if(this.Base == Player)GUIFlashCrosshairs();
                }
            }
        }
        public void doCollision(Vehicle Target, Point3DM ImpactSpot){
            ...
            fragment(ImpactSpot, Target);
            if(this.Base == Player)GUIFlashCrosshairs();
            Target.hit(ImpactSpot, Type.Damage);
    ...
    class Doodad extends Object3DM{
        ...
        public Doodad(double G, double D, Shape3DM S, double Dur, Point3DM Pos, Point3DM Mom, Color T, double Size){
            Drag = (float)D;
            Gravity = (float)G;
            Form = S.copy();
        ...
        public Doodad(double G, double D, char F, double Dur, Point3DM Pos, Point3DM Mom, Color T, double Size){
            Drag = (float)D;
            Gravity = (float)G;
        ...
        public void move(){
            ...
            Acceleration = new Point3DM(0, 0, -Gravity);
            Acceleration.add(Momentum.multiply(-Drag * Momentum.length()), Acceleration);
            Momentum.add(Acceleration.divide(FramesPerSecond), Momentum);
             //   System.out.println(Momentum);
    ...
    */
    ...
    abstract class Vehicle extends Object3DM{
        ...
        HashMap<Projectile, Object3DM> TrackedShots = new HashMap<Projectile, Object3DM>();
        Point3DM Fudge = new Point3DM(0, 0, 0);
        double FudgeDecayFactor = 0.1;
        ...
        public void AILead(){
            ...
            Point3DM Temp = judgeShot(DecisionTarget, SelectedGun.Shot);
            if(Temp == null){
                Firing = false;
                return;
            }else{
            ...
            Point3DM TransformedFudge = new Point3DM();
            DecisionTarget.Form.Rotation.transform(Fudge, TransformedFudge);
            LeadTarget.add(TransformedFudge, LeadTarget);
            updateMissBox();

            if(DecisionTarget.Momentum.length() == 0){
                LeadTarget.add(MissBox.multiply(Form.Center.distanceTo(LeadTarget) / 10), LeadTarget);
            }else{
                LeadTarget.add(MissBox.multiply(Form.Center.distanceTo(LeadTarget)), LeadTarget);
            }
        }
        public void updateMissBox(){
            Point3DM New = Utils3DM.getRandomUnit();
            New.multiply(MissBoxSize / 2 / FramesPerSecond, New);
            MissBox.add(New, MissBox);
            if(MissBox.length() > MissBoxSize){
                MissBox.normalize(MissBoxSize);
            }
        }
        ...
        public void AITrackShots(){
            Set<Projectile> ShotsToRemove = new HashSet<Projectile>();

            for(Projectile Shot : TrackedShots.keySet()){
                Object3DM ShotTarget = TrackedShots.get(Shot);
                if(ShotTarget != DecisionTarget){
                    ShotsToRemove.add(Shot);
                    continue;
                }


                if(Shot.Duration.Current <= 0){//Shot has timed out
                    ShotsToRemove.add(Shot);
                    
                
                }else if(ShotTarget.Form.Center.subtract(Shot.Form.Center).dot(Shot.Momentum) / (ShotTarget.Form.Center.subtract(Shot.LastPosition).dot(Shot.Momentum)) < 0){//shot has passed target
                    ShotsToRemove.add(Shot);
                    Point3DM Direction = Shot.Momentum.copy().normalize();
                    Point3DM CrossPoint = Direction.multiply(ShotTarget.Form.Center.subtract(Shot.LastPosition).dot(Direction)).add(Shot.LastPosition);
                    CrossPoint.subtract(ShotTarget.Form.Center, CrossPoint);
                    ShotTarget.Form.Rotation.inverseTransform(CrossPoint, CrossPoint);
                    if(UseAI)Fudge = Fudge.multiply(1-FudgeDecayFactor).subtract(CrossPoint.multiply(FudgeDecayFactor));
                }
            }

            for(Projectile Shot : ShotsToRemove){
                TrackedShots.remove(Shot);
            }
            
        }
        public Point3DM judgeShot(Object3DM Target, ProjectileType Shot){
            if(Target == this){
                return Utils3DM.Origin;
            }

            double P = Shot.Speed < Momentum.length() ? Momentum.length() + Shot.Speed : Shot.Speed;
            Point3DM V = Shot.Speed < Momentum.length() ? Target.Momentum.copy() : Target.Momentum.subtract(Momentum);
            Point3DM A = Target.Acceleration.subtract(new Point3DM(0, 0, -Shot.Gravity));
            //System.out.println(Shot.Gravity);
            Point3DM S = Target.Form.Center.subtract(Form.Center);
            
            
            double T = GUI.newtonRaphson(0.25 * A.dot(A), 0.5 * V.dot(A), V.dot(V) - P * P + 0.5 * S.dot(A), S.dot(V), S.dot(S), S.length() / (P - S.divide(S.length()).dot(V)), 15, 0.01);
           // double T = GUI.newtonRaphson(0.25 * A.dot(A), V.dot(A), V.dot(V) - P * P - S.dot(A), -2 * S.dot(V), S.dot(S), S.length() / (P - S.divide(S.length()).dot(V)), 20, 0.01);
            if(T == T){
                Point3DM AimPoint = Target.Form.Center.add(V.multiply(T)).add(A.multiply(T*T / 2));
                return AimPoint;
            }else{
                return null;
            }
    ...
    class Turret extends Vehicle{
        ...
        public Turret(String N, Point3DM Position, int S, int GunCount, Gun Weapon){
            ...
            if(Guns.size() > 0){
                ...
                Range = (float)(Guns.gun(0).Shot.Speed * Guns.gun(0).Shot.Duration * RangeMultiple);
            }
            Flag.Tint = getColor(Side);
        ...
        public void initAI(int Level){//Initializes the AI
            ...
            MissBoxSize = 0.4 /(Math.max(0, Level) + 3);
            FireChance = 0.1f + (0.05f) * (Level);
        ...
        public void doAI(){//Uses the AI
            if(!UseAI || Guns.size() == 0 || AILevel < 0 ){ return; }
            AITrackShots();
            AICountdowns();
            AITargetting();
        ...
        public void AIFire(){
            if(Firing && StunCounter.checkBounds() < 0 && !mightIntersectPlane(Form.Center, ControlTarget)){
                ...
                if(getAngleError() < AngleToFire){
                    for(int i = 0; i < Guns.size(); i++){
                        if(Math.random() < FireChance / Guns.gun(i).CooldownCounter.Max / FramesPerSecond){
        ...
        public void AIControl(){
            ...
            double TargetAlpha = Math.PI/2 - Math.acos(GUI.trigCap(vZ.dotProduct(Difference) /  Difference.length()));
        ...
        public void die(){//Destroys craft and generates fragments
            ...
            Doodad[] BigFragment = new Doodad[2];
            //Big
            BigFragment[0] = new Doodad(5, 0, Form, GUI.randomInt(1, 2), Form.Center, Utils3DM.getRandomUnit().multiply(GUI.randomInt(120, 220)).add(Momentum), Color.white, GUI.randomInt(9, 12));
            BigFragment[1] = new Doodad(6, 0, TurretForm, GUI.randomInt(2, 4), Form.Center, Utils3DM.getRandomUnit().multiply(GUI.randomInt(120, 220)).add(Momentum), Color.white, GUI.randomInt(8, 10));
            ...
            Doodads.addAll(BigFragment);
            fragment();
        }
        public void fragment(){
            Doodad[] SmallFragment = createDebris(15, Form.Center, Utils3DM.Origin, null, new Color[]{Color.white, Color.red}, 1, Form.MaxDistance, 1);
            Doodads.addAll(SmallFragment);
        }
        public String getTask(){
  ...
  //      Shape3DM RightWingTrail;
     //   Shape3DM LeftWingTrail;
        ...
        Counter AfterburnerTimer = new Counter(0, 20, 20);
        ...
        float ResumeDistance = 1250; //Distance at which Craft resumes attack after retreating
        ...
        float FireChance = 0.7f; //chance of firing at each opportunity
        ...
        public double HeightFloor(){
            ...
            double HardFloor = 120;
            double CrashTime = 2.5;
			if(this == Player){
				//System.out.println();
				//System.out.println(Momentum.POSZ + "\t" + Form.Center.POSZ);
				//System.out.println(-Math.min(0, Momentum.POSZ) * CrashTime + "\t" + HardFloor);
			}
            return Math.max(-Math.min(0, Momentum.POSZ) * CrashTime, HardFloor);
        }
       ...
       //    public Doodad(boolean G, Shape3DM S, double Dur, Point3DM Pos, Point3DM Mom, Color T, double Size){
            Doodad[] Fragment = new Doodad[2];
            
            //Big Fragments
            Fragment[0] = new Doodad(1, 0, Form, GUI.randomInt(15, 20), Form.Center, Utils3DM.getRandomUnit().multiply(GUI.randomInt(0, 25)).add(Momentum), Form.Tint, CollisionRadius);
            Fragment[0].Form.Points[1].setToMidpoint(Fragment[0].Form.Points[1], Fragment[0].Form.Points[2]);
            Fragment[0].Spin.rotateAround(GUI.getRandomAxis(), Math.PI * Math.random() * 3 * 0.02);

            Fragment[1] = new Doodad(1, 0, Form, GUI.randomInt(15, 20), Form.Center, Utils3DM.getRandomUnit().multiply(GUI.randomInt(0, 25)).add(Momentum), Form.Tint, CollisionRadius);
            Fragment[1].Form.Points[2].setToMidpoint(Fragment[1].Form.Points[1], Fragment[1].Form.Points[2]);
            Fragment[1].Spin.rotateAround(GUI.getRandomAxis(), Math.PI * Math.random() * 3 * 0.02);
            
            //Coloured Fragments
            
            
                
            Doodads.addAll(Fragment);
        }
        public void fragment(){
            Doodad[] SmallFragment = createDebris(15, Form.Center, Utils3DM.Origin, null, new Color[]{Color.white, Color.red}, 150, Form.MaxDistance, 1);
            Doodads.addAll(SmallFragment);
        }
        ...
        //    updateTrails();
            ...
            AfterburnerTimer.setBaseIncrement(0.2);
            ...
            for(int j = 0; j < S.length(); j++){
                ...
                switch(SS){
                    ...
                    case 1: Guns.add(Bomb.copy(this, new Point3DM[] {Form.ActualPoints[1]}, Form.Rotation)); break;
        ...
        public void setAngleOfAttack(){
            if(Speed > 0){
                Point3DM T0 = new Point3DM();
                Point3DM T1 = new Point3DM();
                Point3DM T2 = new Point3DM();
                Momentum.copyTo(T0);
                ...
                AngleOfAttack = Math.asin(GUI.trigCap(DotProduct));
            }else{
            	AngleOfAttack = 0;	
            }
        }
        ...
        public void controls(){
            ...
            final Rotation3DM TempRotation = Utils3DM.getRotation();
            if(MouseHorizontal != MouseHorizontal)throw new IllegalArgumentException("MOUSE NOT DEFINED OMG");
            if(Form.Center.POSZ != Form.Center.POSZ)throw new IllegalArgumentException("MOUSE NOT DEFINED OMG");
                ...
                if(Obj instanceof Position3DM){ //if objective is a position
                    ...
                    if(V.Side == this.Side){ //for follow
                        ...
                        if(T == null){ //if nothing near then circle
                            State = 'c';
                            DecisionTarget = Obj;
                        }else{ //if something near then attack
                        ...
                        if(Obj == null || !Obj.Exists){ //if target invalid then cancel
                            ...
                            if(DecisionTarget != Obj || State == 'c'){ //if new target then attack
                                attackTarget(V);
                            }
        ...
        public void AIDecisions(){
            ...
            if(Form.Center.POSZ < HeightFloor() || (Form.Center.POSZ < HeightFloor() * 2 && TakingOff)){
                State = 't';
                AState = 'r';
                TakingOff = true;
                DecisionTarget = this;
            ...
            switch((Evading && State != 't' ? 'e' : State)){
                case 't': //Take Off
                    ...
                    Momentum.copyTo(ControlTarget);
                    
                    ControlTarget.POSZ = 0;
                    if(ControlTarget.length() == 0){
                    ...
                    ControlTarget.normalize();
                    ControlTarget.multiply(100000, ControlTarget);
                ...
                case 'a': // Attacking Target
                    ...
                    if(AState == 'a'){ //if Attacking
                        ...
                        Form.Center.subtract(DecisionTarget.Form.Center).normalize(10000).add(Form.Center, ControlTarget);
        ...
        public void AICollision(){
            ...
            Point3DM[] NewPath = find(Form.Center, ControlTarget.copy());
            if(NewPath == null){
            	NewPath = find(Form.Center, DecisionTarget.Form.Center.copy());		
            }
            if(NewPath != null){
            	Path = NewPath;	
        ...
        private PriorityQueue<Point3DM[]> PossiblePaths = new PriorityQueue<Point3DM[]>(10, new Comparator<Point3DM[]>(){
            public int compare(Point3DM[] A, Point3DM[] B){return Double.compare(Utils3DM.pathLength(A), Utils3DM.pathLength(B));}
        });
        private Map<Plane, Point3DM[]> ShortestPathTo = new HashMap<Plane, Point3DM[]>();
        public Point3DM[] find(Point3DM Start, Point3DM End){
     		//Check to see if path is impossible
            double Extension = 0;
     		PossiblePaths.clear();
            ShortestPathTo.clear();

            Point3DM[] ClearPath = null;
            PossiblePaths.add(new Point3DM[]{Start, End});


            int i;
            for(i = 0; i < 250 && PossiblePaths.size() > 0 && ClearPath == null; i++){
         
                Point3DM[] Current = PossiblePaths.poll();

                Object[] R = getFirstCollision(Current, Extension);
                if(R == null){//There is a collision but it's with an infinite plane, so no need to keep searching
                }else{
                    
                    Point3DM HitPoint = (Point3DM)R[0];
                    Plane HitPlane = (Plane)R[1];
                    int HitIndex = ((Integer)R[2]).intValue();
               
                    if(HitPoint == null && HitPlane == null){//if there is no collision, then this path is clear; check to see if it is shorter than current clear path
                        if(ClearPath == null || Utils3DM.pathLength(Current) < Utils3DM.pathLength(ClearPath)){
                            ClearPath = Current;
                        }
                    }else if(ShortestPathTo.get(HitPlane) != null && Utils3DM.pathLength(ShortestPathTo.get(HitPlane)) < Utils3DM.pathLength(Current)){//if you are dodging a polygon you already dodged, then this is repeating and should be scrapped.
                    }else if(ClearPath != null && Utils3DM.pathLength(Current) > Utils3DM.pathLength(ClearPath)){ //if you are already longer than the current clear path, then you can only get longer so you should be scrapped
                    }else{
                        ShortestPathTo.remove(HitPlane);
                        ShortestPathTo.put(HitPlane, Current);
                        Point3DM[] Alternatives = avoidPolygon(HitPoint, Current[HitIndex].subtract(Current[HitIndex-1]).normalize(),  HitPlane, Extension);
                    
                        for(Point3DM Alt : Alternatives){
                            if(Alt == null)break;
                            Point3DM[] AltPath = new Point3DM[Current.length+1];
                            System.arraycopy(Current, 0, AltPath, 0, HitIndex);
                            AltPath[HitIndex] = Alt;
                            System.arraycopy(Current, HitIndex, AltPath, HitIndex+1, Current.length - HitIndex);
         ...
         //   if(this == Player)System.out.println(ClearPath);
        //    if(this == Player)System.out.println("i " + i);
            if(this == Player && ClearPath != null)View.print(ClearPath, Color.red);
        ...
        private Object[] GFCReturn = new Object[3];
        public Object[] getFirstCollision(Point3DM[] Path, double Extension){
            Point3DM HitPoint = null;
            Plane HitPlane = null;
            int HitIndex = -1;
            for(int i = 1; i < Path.length; i++){
                Point3DM P1 = Path[i-1];
                Point3DM P2 = Path[i];
                Collection<Plane> PossiblePlanes = PlaneTable.testArea(P1.POSX, P1.POSY, P2.POSX, P2.POSY);
                for(Plane P : PossiblePlanes){
                    
                    Point3DM Hit = P.checkCollision(P1, P2, Extension);
                    if(Hit != null){
                        if(!P.Finite){
                            return null;
                        }else if(HitPoint == null || Hit.distanceTo(Path[0]) < HitPoint.distanceTo(Path[0])){
                            HitPoint = Hit;
                            HitPlane = P;
                            HitIndex = i;
                        }
            ...
            GFCReturn[0] = HitPoint; GFCReturn[1] = HitPlane; GFCReturn[2] = HitIndex;
            return GFCReturn;
        }

        private Point3DM[] APReturn = new Point3DM[10];
        public Point3DM[] avoidPolygon(Point3DM Collision, Point3DM Approach, Plane Target, double Extension){
            if(APReturn.length < Target.Form.ActualPoints.length+1)APReturn = new Point3DM[APReturn.length*2];
            
            for(int i = 0; i < Target.Form.ActualPoints.length; i++){
                Point3DM P1 = Target.Form.ActualPoints[i];
                Point3DM P2 = Target.Form.ActualPoints[(i + 1) % Target.Form.ActualPoints.length];
                Point3DM Edge = P2.subtract(P1);
                Edge.normalize();
                Point3DM EdgeNormal = Edge.crossProduct(Target.Normal);
                EdgeNormal.normalize(P2.subtract(Collision).dot(EdgeNormal) + Extension + 50);
                EdgeNormal.add(Collision, EdgeNormal);
                EdgeNormal.add(Target.Normal.dot(Approach) > 0 ? Target.Normal.multiply(-50) : Target.Normal.multiply(50), EdgeNormal);
                APReturn[i] = EdgeNormal;
            }
            APReturn[Target.Form.ActualPoints.length] = null;
            return APReturn;
        }
        ...
        public void manualFire(){
            if(this == Player && SpaceDown && Player.SelectedGun != null){
                Projectile NewShot = Player.SelectedGun.fire(DecisionTarget);
        ...
        public void AIPickWeapon(){
            ...
            double[] GunPriorities = new double[Guns.size()];
            for(int i = 0; i < GunPriorities.length; i++){
                GunPriorities[i] = (Guns.gun(i).Shot.Damage / Guns.gun(i).CooldownCounter.Max);
                GunPriorities[i] *= (Guns.gun(i).ReloadTimer.Current / Guns.gun(i).ReloadTimer.Max);
                
                GunPriorities[i] += Guns.gun(i).Shot.AIPrefMod;
                
                
                if(DecisionTarget != null && (DecisionTarget.Momentum.length() > Guns.gun(i).Shot.MaxTargetSpeed || DecisionTarget.Momentum.length() < Guns.gun(i).Shot.MinTargetSpeed)){
                    GunPriorities[i] *= 0.1;
                }
                ...
                if(Guns.gun(i) == SelectedGun){
                    GunPriorities[i] = GunPriorities[i] * 3 / 2;
                }
                if(DecisionTarget != null && (DecisionTarget.Momentum.length() > Guns.gun(i).Shot.MaxTargetSpeed || DecisionTarget.Momentum.length() < Guns.gun(i).Shot.MinTargetSpeed)){
                    GunPriorities[i] = 0;
                }
                ...
                if(1 > Guns.gun(i).ReloadTimer.Current){
                    GunPriorities[i] = 0;
                }
            }
            double Max = 0;
        ...
        public void AIJudgeFire(){
            ...
            if(Math.random() > FireChance / SelectedGun.CooldownCounter.Max / FramesPerSecond){
                return;
            }
            ...
            if(Math.acos(GUI.trigCap(TempPoint.dotProduct(Difference) / Difference.length())) < AngleToFire){//If close enough fire
                ...
                if(NewShot != null){
                    TrackedShots.put(NewShot, DecisionTarget);
                }
        ...
        public void AIControls(){
            ...
            if(State == 'a' && AState == 'a' && Path.length == 2){
        ...
        public void cruiseToTarget(){
            Point3DM Diff = ControlTarget.subtract(Form.Center);
            double Theta = Math.acos(GUI.trigCap(Momentum.dot(Diff) / Momentum.length() / Diff.length()));
            Point3DM Delta = (Momentum.cross(Diff)).cross(Diff);
            Delta.normalize(Math.tan(Theta) * Diff.length());
            if(Math.toDegrees(Theta) < 90){
	            ControlTarget.add(Delta, ControlTarget);
    	        pointToTarget();
        	    ControlTarget.subtract(Delta, ControlTarget);
    	 	}else{
        		pointToTarget(); 	
        	}
        ...
        public void pointToTarget(){
            ...
            double AngleToTurn = Math.toDegrees(Math.acos(GUI.trigCap(Difference.dotProduct(MAxis) / (Difference.length()))));
            
            
            //Roll
       
            if(DiffSubM.length() == 0){
            	DiffSubM.POSZ = 0.1;	
            }
            ...
            GUI.checkNaN(RollAmount);
            
            AIMouseTarget.x = GUI.trigCap(-Math.toRadians(RollAmount) / RollSpeedMultiplier / Math.sqrt(Speed) * RollResponsiveness);

            
            //Pitch
            if(DiffSubW.length() == 0){
            	DiffSubW.POSZ = 0.1;	
            }
            
            double PitchAngle = Math.toDegrees(Math.acos(GUI.trigCap(DiffSubW.dotProduct(MAxis) / DiffSubW.length())));
         	GUI.checkNaN(PitchAngle);
            double PitchModulator = Math.abs(Math.cos(Math.toRadians(RollAmount)));//slow down pitching when you haven't rolled to the correct angle yet, to prevent corkscrewing
            AIMouseTarget.y = Math.atan(PitchAngle) * PitchModulator;
        ...
        public void checkEvasion(){
            ...
            for(int i = 0; i < Projectiles.size(); i++){
                ...
                if(P.Form.Center.distanceTo(Form.Center) < P.LastPosition.distanceTo(Form.Center)){
                    ...
                    if(P.Type.Homes && P.Form.Center.distanceTo(Form.Center) / P.Type.Speed < DodgeImpactTime && P.LastPosition.distanceTo(Form.Center) / P.Type.Speed > DodgeImpactTime){
                        Homing = true;
                    }
                }
            }
    ...
    double TurnIncrement = 150;// in degrees / second
    ...
    public void init(){
        ...
        Grid = new Shape3DM(false, Color.GREEN.darker().darker(), 0, 0, 0);
   ...
   //     System.out.println(Thing.FurthestDistance);
    ...
    public void initShots(){
        ...
        //HitRadius = 4
        //Duration = 2s
        {//                        Speed CR DUR   ROT  SPREAD DMG 
        BombShot = new ProjectileType(0, 4, 200.0, true, 0.01, 4.0);
        BombShot.setTrail(2, 3, 8);
        BombShot.setGravity(10);
        BombShot.setAreaOfEffect(75);
        Shape3DM Form = new Shape3DM(false, Color.red, 0, 0, 0);
        Utils3DM.setPoints(Form, 'o', 1, 1, 1);
        Form.Rotation.rotateAround('y', Math.toRadians(90));
        Utils3DM.center(Form, 'l');
        BombShot.setShape(Form);
        BombShot.setAIData(3450, 0, 50, 0, 0);
        for(int i = 0; i < BombShot.Form.Points.length; i++){
            Form.Rotation.transform(BombShot.Form.Points[i], BombShot.Form.Points[i]);
        }
        BombShot.Form.Rotation.identify();

        BombShot.FireSound = (WaveSound)Base.SoundMap.get("Shot - MachineGun");;
        }
        ...
        //Duration = 2s
        {//                                 Speed CR DUR   ROT  SPREAD DMG 
        MachineGunShot = new ProjectileType(1050, 3, 3.0, true, 0.015, 0.4);
        MachineGunShot.setTrail(0.04, 3, 3);
        MachineGunShot.setGravity(0);
        Shape3DM Form = new Shape3DM(false, Color.red, 0, 0, 0);
        Utils3DM.setPoints(Form, 'p', 0.3, 0.9, 0);
        Form.Rotation.rotateAround('y', Math.toRadians(90));
        Utils3DM.center(Form, 'l');
        MachineGunShot.setShape(Form);
        MachineGunShot.setAIData(1850, 0, 600, 0, 0);
        ...
        //Duration = 3s
        {
        MissileShot = new ProjectileType(600, 5, 7.0, true, 0.1, 4.0);
        MissileShot.setTrail(1, -1, 10);
        ...
        MissileShot.setAIData(2800, 750, 400, 0, 2);
        ...
        //Cannon Shot
        {
        CannonShot = new ProjectileType(1500 , 6, 3.0, true, 0.002, 5.0);
        CannonShot.setTrail(0.5, 1, 3);
        CannonShot.setGravity(10);
        Shape3DM Form = new Shape3DM(false, Color.red, 0, 0, 0);
        Utils3DM.setPoints(Form, 'o', 6, 2, 2);
        Utils3DM.center(Form, 'p');
        CannonShot.setShape(Form);
        CannonShot.setAIData(1500, 0, 200, 0, 1);
        CannonShot.FireSound = (WaveSound)Base.SoundMap.get("Shot - Cannon");
        }
        //MachineGun
        //MaxBurst 2s
        //ReloadTime 8s
        MachineGun = new Gun("Machine Gun", null, null, null, MachineGunShot, -1337, 40, 4, 0.05);
        
        //MissileLauncher
        //MaxBurst 2 shots
        //ReloadTime 12.5s
        MissileLauncher = new Gun("Missile", null, null, null, MissileShot, -1337, 2, 0.05, 0.1);
        //Cannon
        //MaxBurst 1 shot
        //Reload Time 10s
        Cannon = new Gun("Cannon", null, null, null, CannonShot, -1337, 1, 0.05, 0.1);

        //Bomb
        //MaxBurst 1s
        //Reload Time 10s
        Bomb = new Gun("Bomb", null, null, null, BombShot, -1337, 5, 0.2, 0.3);
        
        GunList = new Gun[]{MachineGun, Bomb, MissileLauncher};
    }
    private void initStars(String[][] Input){

        double Latitude = 0;
        double Longitude = 0;
        Vector<Sprite3DM> StarBuffer = new Vector<Sprite3DM>();
        
        int i = 0;
        for(String[] Line : Input){
            double RA1 = Double.parseDouble(Line[2]);
            double RA2 = Double.parseDouble(Line[3]);
            double Dec = Double.parseDouble(Line[4]);
            double RAA = Math.toRadians(15 * RA1 + 15 * RA2 / 60);
            double DecA = Math.toRadians(Dec);
            double VisualMagnitude = Double.parseDouble(Line[8]);
            Point3DM StarPosition = new Point3DM(1000000000, 0, 0);

            Rotation3DM R = new Rotation3DM();
            R.rotateAround('y', DecA);
            R.transform(StarPosition, StarPosition);
            R.rotateAround('z', RAA);
            R.transform(StarPosition, StarPosition);
            R.rotateAround('y', Math.toRadians(90));

            //int VisualMagnitude = (int)(100 * (5-Double.parseDouble(Line[8])));
            int Brightness = (int)(2000 / Math.pow(2, VisualMagnitude));
            System.out.println(Brightness + "\t" + VisualMagnitude);
            int B = GUI.cap(0, Brightness, 255);
            Shape S = new Rectangle(0, 0, 0, 0);
            Sprite3DM NewStar = new Sprite3DM(S, StarPosition, new Color(B, B, B));
            NewStar.Scaling = false;
            StarBuffer.add( NewStar);
            if(Brightness > 255){
                B = GUI.cap(0, (Brightness - 255) / 4, 200);
                S = new Polygon(new int[]{-1, 0, 1, 0}, new int[]{0, 1, 0, -1}, 4);
                NewStar = new Sprite3DM(S, StarPosition, new Color(B, B, B));
        ...
        for(Sprite3DM Star : StarBuffer){
            if(Star.Center.POSZ < 0){
                Star.Tint = Color.black;
            }
        }
        Stars = new Sprite3DM[StarBuffer.size()];
        StarBuffer.toArray(Stars);
        ...
        try{
            ...
            for(int i = 0; i < N; i++){
             ...
             //   System.out.println(i + " " + Arrays.toString(NewPlane));
                ...
                ((Plane)Planes.lastElement()).Form.Tint = Color.white.darker();
            ...
            for(int i = 0; i < N; i++){
                ...
                if(Type == 'c'){ //if it is a craft
                    ...
                    Gun[] Guns = {MachineGun, Bomb, MissileLauncher};
    ...
    int Index = -1;;
    GameState ExitState;
    public FlightState(FlightWindow F, String DataPath, String[][] Stardata, int I, GameState ES){
        ...
        Stopwatch = new Timer((int)(15), this);
        Stopwatch.stop();
        //SET CONTENT PANE
    ...
    public void togglePlayerTarget(int Side){
        ...
        }while((((Player.DecisionTarget.Side == Player.Side) != (Player.Side == Side)) || Player.DecisionTarget.Form.Center.distanceTo(Player.Form.Center) > TargetRange) && Count > 0);
        if(Count == 0){
    ...
    public void doStuff(){
    	for(int i = 0; i < Vehicles.size(); i++){
    		if(Vehicles.ve(i) instanceof Craft && Vehicles.ve(i).Side == Player.Side){
    			System.out.println(Arrays.toString(((Craft)Vehicles.ve(i)).Path));
    		}	
    	}	
    }
    public void keyPressed(KeyEvent Event){
...
//            case 61: ThrottleUp = !true; break; //=
    ...
    Map<Integer, LinkedList<Doodad>> DebrisStore = new HashMap<Integer, LinkedList<Doodad>>();
    //Explosiveness: Fragment Speed Multiplier, default = 1
    //TSize: Linear Dimension to be converted into Volumetric Bulk, which is split into each individual fragment; ranges from 1 (for machinegunshot) to 10+ (for bombshot)
    //Burstiness: higher values multiply fragment speed and fragment drag, resulting in explosions that reach their maximum radius much faster. Also divides the average fragment size, resulting in a much smaller mix of fragments
    public Doodad[] createDebris(int Duration, Point3DM Position, Point3DM Momentum, Point3DM SurfaceNormal, Color[] Tint, double Explosiveness, double TSize, double Burstiness){
        java.util.List<Doodad> Output = new LinkedList<Doodad>();
        for(int TBulk = Math.max(1, (int)(TSize*TSize*TSize)), FBulk = (int)(1.0*TBulk/6/Burstiness) + 1; TBulk > 0; TBulk -= FBulk){
            ...
            double FSize = Math.pow(FBulk, 1.0/3);
            Doodad D = new Doodad(100, 0.01 * Burstiness / (FSize+1), A, Math.random()*Duration, Position, RandomUnit.multiply(Math.random() * TSize * Explosiveness * 20 * Burstiness), Tint[GUI.randomInt(0, Tint.length-1)], FSize);
            System.out.println(SurfaceNormal);
            if(SurfaceNormal != null)D.Form.Center.add(SurfaceNormal.multiply(0.1), D.Form.Center);
            if(SurfaceNormal != null && SurfaceNormal.dot(D.Momentum) < 0)D.Momentum.add(SurfaceNormal.multiply(-2*SurfaceNormal.dot(D.Momentum)), D.Momentum);

            Output.add(D);
            D.Spin.rotateAround(GUI.getRandomAxis(), Math.PI * Math.random() * 3 * 0.02);


        }
        return Output.toArray(new Doodad[Output.size()]);
    }/*
    public Doodad[] createDebris(int Duration, Point3DM Position, double Explosiveness, Point3DM Momentum, Color[] Tint, int Count, int Size){
        Doodad[] Output = new Doodad[Count];
        for(int i = 0; i < Count; i++){
            Output[i] = dcreateDebris(Duration, Position, Explosiveness, Tint[0]);
        }
        return Output;
    }*/
    ...
    public void alignViewpoint(Craft Target){
        ...
        double Difference = GUI.angleDiff(CurrentDirection, 360.0 / DirectionCount * GUI.cap(0, Math.abs(Direction) - 1, DirectionCount - 1));
        
        if(Math.abs(Difference) < 360.0 / DirectionCount / 16){
    ...
    public void actionPerformed(ActionEvent Event){
        System.out.println(Player.UseAI);
    ...
    //   System.out.println(Trails.size() + "\t" + Projectiles.size());
        ...
        for(int i = Projectiles.size() - 1; i >= 0; i--){
            ...
            Collection<Plane> PossiblePlanes = PlaneTable.testPoint(P.Form.Center.POSX, P.Form.Center.POSY, P.Form.Center.distanceTo(P.LastPosition));
            //Object[] PossiblePlanes = PlaneTable.testPoint(P.Form.Center.POSX, P.Form.Center.POSY);
            for(Plane Pl : PossiblePlanes){
        ...
        for(int i = Doodads.size() - 1; i >= 0; i--){
            ...
            Collection<Plane> PossiblePlanes = PlaneTable.testPoint(D.Form.Center.POSX, D.Form.Center.POSY);
            for(Plane Pl : PossiblePlanes){
                ImpactSpot = (Pl).checkCollision(Doodads.d(i));
                if(ImpactSpot != null){
                    (Pl).doCollision(Doodads.d(i), ImpactSpot);
                }
        ...
        for(int j = 0; j < Vehicles.size(); j++){
            ...
            Collection<Plane> PossiblePlanes = PlaneTable.testPoint(V.Form.Center.POSX, V.Form.Center.POSY, V.CollisionRadius);

                
            for(Plane Pl : PossiblePlanes){

                ImpactSpot = (Pl).checkCollision(Vehicles.ve(j));
                if(ImpactSpot != null){   
                    (Pl).doCollision(Vehicles.ve(j), ImpactSpot);
        ...
        if(EndCounter.checkBounds() == 0){
            ...
            Area.printLine(X/2, Y/2 + 40, "Press Esc to continue", 20, Color.green, 'c');
    ...
    public void checkMissionAccomplished(){
     //   System.out.println(EndCounter);
        ...
        if(LoseCondition.checkComplete() == 2 || !Player.Exists){
            Result = "Failed";
            prepareToEnd();
   ...
   // 		}
     ...
     // 	}
 //       Player.find(Player.Form.Center, Player.DecisionTarget.Form.Center);
     //	for(int i = 0; i < Vehicles.size(); i++){
     // 		Vehicle V = Vehicles.ve(i);
    ...
    public void GUIBase(){
        ...
        */
        ...
        if(FlashCounter.Current == 0){
            Area.Painter.setColor(Color.green);
        }else{
            Area.Painter.setColor(Math.random() > 0.5 ? Color.red : Color.white);
        }
        CenterGap /= 2;
    ...
    public void GUITargeting(){
        ...
        if(FlashCounter.Current == 0){
            ...
            Area.Painter.setColor(Math.random() > 0.5 ? Color.red : Color.white);
            FlashCounter.increment(-1.0 / FramesPerSecond);
            FlashCounter.cap();
        }
        if(Player.DecisionTarget != null && Player.DecisionTarget.Exists && Player.SelectedGun != null && Player.DecisionTarget != Player && Player.DecisionTarget.Form.Center.distanceTo(Player.Form.Center) < Player.SelectedGun.Shot.MaxEffectiveRange * 1.25){
            final Point3DM Target = new Point3DM();;
            ...
            Point3DM Lead = Player.judgeShot(Player.DecisionTarget, Player.SelectedGun.Shot);
           ...
           // Target.subtract(View.POS, ObserverVector);
            ...
            if(Lead != null && Lead.subtract(View.POS).dot(View.LookVector) > 0){
                Target.center(View, TransformedTarget);
                FlatPoint.setLocation(0, 0);
                TransformedTarget.toScreen(View, FlatPoint);
                int FX = (int)FlatPoint.x; int FY = (int)FlatPoint.y;
                Area.Painter.drawLine(FX-10, FY-10, FX-5, FY-5);
                Area.Painter.drawLine(FX+10, FY-10, FX+5, FY-5);
                Area.Painter.drawLine(FX-10, FY+10, FX-5, FY+5);
                Area.Painter.drawLine(FX+10, FY+10, FX+5, FY+5);
    ...
    public void GUIFlashCrosshairs(){
        FlashCounter.Current = FlashCounter.Max;
    }
    public Counter FlashCounter = new Counter(0, 0.5, 0);
    ...
    public void GUIHitpoints(){
      ...
      //  System.out.println(Player.Hitpoints);
        ...
        Area.Painter.drawRect((int)(0.9 * Area.W - Length + 15) + (int)(Length * GUI.cap(0, Player.Hitpoints.Current, Player.Hitpoints.Max) / Player.Hitpoints.Max) - 2, (int)(0.75 * Area.H + -2 * (Width + Gap)) + 2, 0, Width - 4);
    ...
    public void GUIWeapons(){
        ...
        for(int i = 0; i < Player.Guns.size(); i++){
            Area.Painter.setColor(Player.DecisionTarget != null && Player.DecisionTarget.Form.Center.distanceTo(Player.Form.Center) < Player.Guns.gun(i).Shot.MaxEffectiveRange ? Color.green : Color.yellow);
            ...
            Area.printLine((int)(0.9 * Area.W - Length + 15 + 1), (int)(0.75 * Area.H + (i + 0.5) * (Width + Gap) - 1), Player.Guns.gun(i).Name, 10, Area.Painter.getColor(), 'l');
      ...
      //  GUIView = View.copy();
        ...
        if(Player.DecisionTarget != null && Player.DecisionTarget.Exists){
            if(Player.DecisionTarget instanceof Vehicle){
                ...
                double Alpha = Math.asin(GUI.trigCap(Diff.dotProduct(Utils3DM.Vectork)));
    ...
    public boolean mightIntersectPlane(Point3DM A, Point3DM B){
        Collection<Plane> PossiblePlanes = PlaneTable.testArea(A.POSX, A.POSY, B.POSX, B.POSY);
        for(Plane Pl : PossiblePlanes){
            if((Pl).checkCollision(A, B) != null){
                return true;
            }
        }
