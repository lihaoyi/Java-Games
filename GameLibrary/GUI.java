...
public class GUI{
    ...
    */
    static HashMap<String, WaveSound> HM = new HashMap<String, WaveSound>();
    public static void playSound(boolean Play, String filename, int LoopCount, int multiplier){
        try{
            AudioStream as = new AudioStream(new FileInputStream("Sounds/"+filename));
        ...
        if(HM.containsKey(filename)){
            HM.get(filename).play();
        }else{

            WaveSound W = new WaveSound("Sounds/"+filename,  1000000);
