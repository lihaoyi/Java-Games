...
public class FlatHash{
    ...
    private final Vector ReturnVector = new Vector();
    private final Set ReturnValues = new HashSet();
    ...
    public Vector testArea(double XA, double YA, double XB, double YB){
        if(!Finalized){
        ...
        for(int x = iX; x <= aX; x++){
            for(int y = iY; y <= aY; y++){
                ...
                for(int i = 0; i < Objects[x][y].length; i++){
                    ReturnValues.add(Objects[x][y][i]);
                }
            }
        }
     //   System.out.println(tX + " " + tY + "\t" + iX + " " + iY + "\t" + aX + " " + aY);
        ReturnVector.clear();
        ReturnVector.addAll(ReturnValues);
        return ReturnVector;
    }
    public Vector testPoint(double X, double Y, double R){
    ...
    public Vector testPoint(double X, double Y){
        ...
        if(tX < 0 || tY < 0 || tX >= Count || tY >= Count){
            ...
            for(int i = 0; i < Objects[tX][tY].length; i++){
                ReturnVector.add(Objects[tX][tY][i]);
            }
        }
        return ReturnVector;
    }
