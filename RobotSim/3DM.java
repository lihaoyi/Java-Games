...
class Utils3DM{
                ...
                for(double i = -0.5; i <= 0.5; i+=1){
                    for(double j = -0.5; j <= 0.5; j+=1){
                        for(double k = -0.5; k <= 0.5; k+=1){
                            CPoints.add(new Point3DM(CurrentShape, i * ArgA, j * ArgB, k * ArgC));
                        }
                    }
                }
    ...
    public static void center(Shape3DM Form, char Switch){
        ...
        for(int i = 0; i < Form.BasePoints.size(); i++){
            Form.MaxDistance = Math.max(Form.BasePoints.point3DM(i).distanceTo(Utils3DM.Origin), Form.MaxDistance);
        }
        for(int i = 0; i < Form.Points.length; i++){
...
class Sprite3D{
    ...
    class Shape3DM{
        ...
        public Shape3DM(int PC){
            ...
            Points = new Point3DM[PC];
            ActualPoints = new Point3DM[PC];
            for(int i = 0; i < PC; i++){
                TransformedPoints[i] = new Point3DM(0, 0, 0);
                FlatPoints[i] = new Point2D.Double();
                Points[i] = new Point3DM(0, 0, 0);
        ...
        public Shape3DM(){
            ...
            Tint = Color.white;
            Static = true;
            ActualPoints = new Point3DM[0];
            TransformedPoints = new Point3DM[0];
        ...
        public void toBytes(DataOutputStream Output) throws IOException{
        ...
        public void fromBytes(DataInputStream Input) throws IOException{
    ...
    final double[][] TempMatrix = new double[3][3];
...
class Point3DM{
    ...
    public void toBareBytes(DataOutputStream Output) throws IOException{
        Output.writeFloat((float)POSX); //1
        Output.writeFloat((float)POSY); //2
        Output.writeFloat((float)POSZ); //3
    }
    public void fromBareBytes(DataInputStream Input) throws IOException{
...
class ObserverM{
 ...
 //   Rotation3D InverseRotation = new Rotation3D();
    ...
    Shape3DM LineShape = new Shape3DM(10);
    public void print(Point3DM[] ActualPoints, Color Tint, Graphics2D Painter){
        for(int i = 0; i < ActualPoints.length; i++){
            ActualPoints[i].copyCoords(LineShape.ActualPoints[i]);
    ...
    public void toBytes(DataOutputStream Output, ShapeVectorM Shapes) throws IOException{
        POS.toBareBytes(Output); //1
        Rotation.toBytes(Output); //2
        for(int i = Shapes.size() - 1; i >= 0; i--){
            if(Shapes.shape3DM(i).checkForBehind(this)){
                Shapes.removeElementAt(i);
            }
        }
        Output.writeInt(Shapes.size()); //3
        for(int i = 0; i < Shapes.size(); i++){ //4
            Shapes.shape3DM(i).toBytes(Output);
        }
        Output.flush();
        
    }
    public void fromBytes(DataInputStream Input, ShapeVectorM Shapes) throws IOException{
        ...
        for(int i = 0; i < ShapeCount; i++){ //4
            ...
            Shapes.shape3DM(i).fromBytes(Input);
        }
